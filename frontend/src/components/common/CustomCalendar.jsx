...
import leftArrowIcon from "../../assets/image/common/left_arrow.png";
import rightArrowIcon from "../../assets/image/common/right-arrow.png";

/**
 * CustomCalendar - 연/월 이동 및 선택 기능이 추가
 * [수정] 여러 건의 예약을 표시하기 위해 reservations 배열을 받도록 확장
 * [추가] 예약 일자에 빨간 동그라미 표시 및 호버 시 시간/장소 툴팁 제공
 * [변경] 화살표 버튼을 텍스트에서 이미지 아이콘으로 변경
 */
const CustomCalendar = ({
                            selectedDate,
                            onDateClick,
                            reservations = [], //전체 예약 데이터 리스트
                            isModal = false
                        }) => {
    ...
    for (let d = 1; d <= lastDate; d++) {
        ...
        //해당 날짜(d)에 해당하는 모든 예약 찾기 (PENDING, CONFIRMED 상태인 것만)
        ...
        days.push(
            <div
                key={`day-${year}-${month + 1}-${d}`} // 날짜별 고유 키값 생성
    ...
    return (
        <div className="calendar-container" style={{ width: '100%' }}>
            <div className="cal-header" style={{
                ...
                <button
                    ...
                    style={{
                        ...
                        display: 'flex',
                        alignItems: 'center',
                        opacity: (isModal && month === today.getMonth() && year === today.getFullYear()) ? 0.3 : 1
                    }}
                    disabled={isModal && month === today.getMonth() && year === today.getFullYear()}
                >
                    <img src={leftArrowIcon} alt="이전달" style={{ width: '1rem', height: '1rem' }} />
                </button>
                ...
                >
                    <img src={rightArrowIcon} alt="다음달" style={{ width: '1rem', height: '1rem' }} />
                </button>
            </div>
            ...
            >
                {/*요일 이름 배열 매핑 시 인덱스를 사용하여 중복 키(S, T) 해결 */}
