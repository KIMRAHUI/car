        ...
        if (isEditMode) {
            axios.get(`/api/review/${reservation.reviewId}`)
                .then(res => {
                    ...
                    // 접속 환경에 따른 이미지 베이스 주소 결정
                    const host = window.location.hostname;
                    const imageBaseUrl = host === 'localhost' ? 'http://localhost:8080' : 'https://car.rhui.dev';

                    setPreviews([
                        data.image1 ? `${imageBaseUrl}${data.image1}` : null,
                        data.image2 ? `${imageBaseUrl}${data.image2}` : null
                    ]);
                })
