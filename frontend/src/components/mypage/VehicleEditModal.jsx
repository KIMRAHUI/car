...
const VehicleEditModal = ({ onClose, onSelectComplete, onSuccess, isRegisterMode = false }) => {
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (data.result === 'success') {
                            //window.location.reload()를 제거, onSuccess 콜백을 통해 데이터를 갱신
                            showAlert(
                                "수정 완료",
                                "차량 정보가 성공적으로 변경되었습니다.",
                                () => {
                                    if (onSuccess) onSuccess(); // MyPage의 fetchUserInfo 실행
                                    onClose(); // 모달 닫기
                                }
                            );
                        } else {
