...
const ReservationModal = ({ partner, onClose, userEmail }) => {
    ...
    useEffect(() => {
        const fetchCurrentSession = async () => {
            try {
                // [수정 핵심] 마이페이지와 동일하게 가장 정확한 /user/status를 호출합니다.
                const res = await axios.get('/user/status');

                // [수정 핵심] result === 'success' 대신 isLoggedIn 필드를 확인합니다.
                if (res.data.isLoggedIn === true) {
                    // 서버가 준 데이터 전체(email, name 등 포함)를 currentUser에 담습니다.
                    setCurrentUser(res.data);
                } else {
                    console.warn("로그인 세션이 유효하지 않습니다.");
                }
            } catch (e) {
    ...
    const calculateTotalRating = () => {
        // 업체 고유 ID 기반 알고리즘 점수 생성 (30% 비중)
        // 데이터가 아예 없을 때만 100% 노출되고, 리뷰가 생기면 보조 수단으로 전환
        const idBase = parseInt(partner.id?.slice(-3) || '123') % 9;
        const externalRating = 4.0 + (idBase * 0.1);

        // 우리 플랫폼 실제 사용자 리뷰 평균 계산 (70% 비중)
        const internalRating = reviews.length > 0
            ? reviews.reduce((acc, cur) => acc + cur.rating, 0) / reviews.length
            : 0;

        // 최종 신뢰 지수 산출
        // 실제 리뷰가 하나라도 있다면 사용자 점수 7, 알고리즘 점수 3의 비율로 섞음
    ...
    const handleSubmit = async () => {
        ...
        if (category !== '일반' && uploadImages.length > 0) {
            uploadImages.forEach((imgObj) => {
                formData.append('images', imgObj.file); // 키값을 'images'로 정확히 지정
            });
        }
                    ...
                    <div className="res-body">
                        ...
                        {step === 1 && (
                            <div className="step-container step-1">
                                ...
                                {/* 4. 탭별 상세 내용 영역 */}
                                <div className="tab-content" style={{ minHeight: '200px',padding: 0 }}>
                                    {tab === 'intro' ? (
                                        ...
                                        <div className="review-list" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                                            {isLoadingReviews ? (
                                                ...
                                                reviews.map((r) => (
                                                    <div key={r.id} className="review-item" style={{ borderBottom: '1px solid #eee', padding: '12px 0', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>

                                                        {/* 1. 왼쪽: 후기 이미지 (이미지가 있을 때만 공간 차지) */}
                                                        {(r.image1 || r.image2) && (
                                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', flexShrink: 0 }}>
                                                                {r.image1 && <img src={r.image1} alt="review-1" style={{ width: '60px', height: '60px', objectFit: 'cover', borderRadius: '4px' }} />}
                                                                {r.image2 && <img src={r.image2} alt="review-2" style={{ width: '60px', height: '60px', objectFit: 'cover', borderRadius: '4px' }} />}
                                                            </div>
                                                        )}

                                                        {/* 2. 오른쪽: 텍스트 및 태그 영역 */}
                                                        <div style={{ flex: 1, minWidth: 0 }}>
                                                            <div className="review-top" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                                                            ...
                                                            {/* 후기 태그 영역 */}
                                                            {r.selectedTags && r.selectedTags.length > 0 && (
                                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                                                                    {r.selectedTags.map((tag, idx) => (
                                                                        <span key={idx} style={{
                                                                            background: '#fff4ce',
                                                                            color: '#856404',
                                                                            fontSize: '0.7rem',
                                                            ...
                                                            <div style={{ fontSize: '0.65rem', color: '#bbb', textAlign: 'right', marginTop: '5px' }}>
