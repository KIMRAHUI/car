...
public class ReviewService {
    ...
    /**
     * 리뷰 관련 이미지 저장 경로 반환 (OS별 구분자 자동 처리)
     */
    private String getReviewPath() {
        return uploadBaseDir + "review" + File.separator;
    }

    /**
     * [READ] 모든 후기 목록 조회 (랜딩페이지 및 리스트용)
     * 각 리뷰에 연관된 태그 리스트를 함께 채워서 반환
     */
    public List<Review> getAllReviews() {
        // 모든 리뷰 기본 정보 조회
        List<Review> reviews = this.reviewMapper.selectAllReviews();

        // 각 리뷰마다 태그 리스트 조회 후 셋팅
    ...
    public Result updateReview(ReviewRequest request) {
        ...
        try {
            ...
            // 이미지 처리 (새로 업로드된 이미지가 있으면 교체, 없으면 기존 경로 유지)
            List<String> savedPaths = saveImages(request.getImages());
    ...
    private List<String> saveImages(List<MultipartFile> images) throws IOException {
        ...
        String fullPath = getReviewPath(); // 동적으로 경로 획득
    ...
    public List<Review> getReviewsByPartner(String partnerId) {
        // 매퍼를 통해 해당 업체 ID를 가진 예약들의 리뷰 리스트를 가져옴
