...
public interface UserMapper {
    ...
    /**
     * [READ] 모든 차량 브랜드 목록을 조회 (모달 Step 1용)
     */
    List<Map<String, Object>> selectAllBrands();

    /**
     * [READ] 특정 브랜드에 속한 차량 모델 목록을 조회 (모달 Step 2용)
     */
    ...
    /**
     * [UPSERT] 통합 정비 이력 테이블에 데이터를 저장하거나 업데이트
     * @param email 사용자 이메일
     * @param itemName 항목명 (예: 엔진오일)
     * @param lastServiceDate 정비 날짜
     * @param lastServiceMileage 정비 당시 주행거리
     * @param source 기록 출처 (MANUAL 또는 RESERVATION)
     * @return 영향받은 행의 수
     */
    int upsertMaintenanceRecord(@Param("email") String email,
                                @Param("itemName") String itemName,
                                @Param("lastServiceDate") String lastServiceDate,
                                @Param("lastServiceMileage") int lastServiceMileage,
                                @Param("source") String source);



    /**
     * [READ] 마스터 정비 항목과 사용자의 마지막 정비 이력을 조인하여 전체 상태를 조회
     * - XML의 selectAllMaintenanceStatus 쿼리와 매핑
     * @param email 사용자 이메일
